...
class PVObject(object):
    ...
    def compute_preds_in_star_basis(self, constraint_list, error_star):
        safe_basis_predicates = []
        # print(safe_constraint_list)
        for std_pred in constraint_list:
            # Translating usafe set star into the simulation/star's basis
            new_lc_vector = np.dot(error_star.basis_matrix, std_pred.vector)
            new_lc_value = std_pred.value - np.dot(error_star.center, std_pred.vector)
            safe_basis_predicates.append(LinearConstraint(new_lc_vector, new_lc_value))

        for constraint in error_star.constraint_list:
            safe_basis_predicates.append(constraint)

        return safe_basis_predicates
    ...
    def powerset(iterable):
        s = list(iterable)
        start_time = time.clock()
        # for comb in combinations(s, len(s)):
            # i = 1

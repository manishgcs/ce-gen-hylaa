...
def define_ha(settings, usafe_r=None):
    ...
    a_matrix = np.array([[2.6629, -1.1644, 0.66598], [2, 0, 0], [0, 0.5, 1]], dtype=float)

    b_matrix = np.array([[0.25], [0], [0]], dtype=float)
    ...
    loc1.c_vector = np.array([0.0, 0.0, 0.0], dtype=float)
    # print(a_bk_matrix)
    ...
    if usafe_r is None:
        ...
        # exp 1
        usafe_set_constraint_list.append(LinearConstraint([0.0, 1.0, 0.0], -8.3))
...
if __name__ == '__main__':
    ...
    init_r = HyperRectangle([(0.9, 1.0), (0.9, 1.9), (0.9, 1.9)])

    pv_object = run_hylaa(settings, init_r, None)

    # longest_ce = pv_object.compute_longest_ce()

    # mid-order = +3 (+4 is better)
    # random: [16, 19, 18, 26, 11, 22, 12, 14, 9, 7, 17, 2, 15, 1, 6, 13, 0, 5, 10, 3, 20, 25, 8, 4, 24, 21, 23]
